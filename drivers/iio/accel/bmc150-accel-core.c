...
static irqreturn_t iio_bmi323_trigger_h(int irq, void *p)
{
	...
	const int available_words_read_res = bmi323_read_u16(&indio_data->bmi323, BMC150_BMI323_FIFO_FILL_LEVEL_REG, &available_words);
	...
	*/
	...
	for (u16 f = 0; f < available_frame_aggregates; ++f) {
		...
		if (indio_data->bmi323.i2c_client != NULL) {
			...
			int read_block_ret = i2c_smbus_read_i2c_block_data(indio_data->bmi323.i2c_client, BMC150_BMI323_FIFO_DATA_REG, bytes_to_read, &fifo_data[0]);
